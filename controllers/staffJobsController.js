...
const getCashPaymentList = async (req, res) => {
    try {
        const staffId = req.user.id;
        const bookings = await getCashPaymentBookings(staffId);
...
const validateCashPaymentController = async (req, res) => {
    try {
        const staffId = req.user.id;
        ...
        const result = await validateCashPayment(booking_id, staffId, amountReceived, payment_note);
...
const getCashPaymentStats = async (req, res) => {
    try {
        const staffId = req.user.id;
        const stats = await getStaffCashPaymentStats(staffId);
        ...
        res.json({
            ...
            stats: {
                ...
                today_revenue: parseFloat(stats[0]?.today_revenue) || 0
            }
        });
...
const getCashPaymentHistory = async (req, res) => {
    try {
        ...
        const history = await getStaffCashPaymentHistory(staffId, limit);
